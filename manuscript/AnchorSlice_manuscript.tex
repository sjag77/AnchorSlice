...
Arm~A is how a source-level detector is used in practice: the complete contract is submitted with a short instruction naming the eight categories, one line each, and asking the detector to report a fault where it is reachable and exploitable. Arm~B is AnchorSlice with both of its components: the slice, preceded by a short preamble explaining the block format and stating that the category tags are lexical hints rather than findings, together with the anchor-derived decision rules of Section~\ref{sec:anchorslice}. The detector, reasoning effort, output schema and contracts are identical, with one call each, so the comparison measures what the method contributes over ordinary practice. Because both arms classify the same 1,600 cells, the comparison is paired, and we report McNemar's exact test on discordant cells alongside the aggregate measures. Section~\ref{subsec:effectiveness} then attributes the outcome to each component in turn.
...
Two observations bound what each component contributes. First, the 130 contracts of the last sampling round were never inspected while the method was developed; on those alone AnchorSlice reaches an F1-score of 0.689 and an agreement of 0.797, against 0.340 and 0.634 for ordinary practice ($p \approx 4.1 \times 10^{-18}$), so the result is not an artefact of development on the evaluation set. Second, the two components act on different quantities and their effects are separable: the slicer determines how much the detector reads, and a pilot in which only the input changed under a fixed prompt left detection statistically unchanged while still cutting tokens, whereas the decision rules determine how the detector judges what it reads and account for the movement in Table~\ref{tab:effectiveness}. Neither component is sufficient alone -- rules without a slice leave the token cost untouched, a slice without rules leaves the detector guessing what the benchmark means by each category -- and together they deliver both results at once.
...
\noindent\emph{Attribution between the two components.} Arm~B exercises both components of AnchorSlice, so the headline figures describe the method as a whole. The separation we report is grounded in a pilot on a 70-contract subset in which only the input changed under a fixed prompt: the slice left detection statistically unchanged while still reducing token consumption, which places the token saving with the slicer and the detection gain with the decision rules. A component-level ablation across the full sample, and a tag-free slice that separates removing code from naming candidate categories, would quantify that split more precisely.
