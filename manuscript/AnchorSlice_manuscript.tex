...
\begin{figure*}[htbp]
    ...
    \caption{AnchorSlice and the practice it replaces. Arm~A hands the detector a contract as it is, with a one-line list of the categories. Arm~B is AnchorSlice: one anchor set per DASP category drives two components, a static slicer that shrinks the input and a set of anchor-derived decision rules that state what each category means in this corpus. The detector, Claude Opus~5, and the output schema are identical in both arms.}
    \label{fig:pipeline}
\end{figure*}
