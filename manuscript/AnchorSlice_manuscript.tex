...
\begin{abstract}
Automated vulnerability detection is a prerequisite for deploying smart contracts safely, because a deployed contract is immutable and every defect in it remains reachable by an adversary. Detectors that reason directly over source code have become increasingly capable, but their cost, latency and context capacity are bounded by the number of tokens they read, and a typical Solidity contract is dominated by comments, library code and boilerplate that cannot host a vulnerability. This paper presents AnchorSlice, a static, model-free stage that makes smart contract vulnerability detection token-efficient. From one anchor set per category of the DASP taxonomy it builds two components: a slicer that reduces each contract, before it reaches the detector, to the fragments capable of hosting each category, and a set of anchor-derived decision rules that tell the detector what each category means in the corpus at hand. AnchorSlice is purely lexical: category-specific anchor expressions and brace matching decide what survives, overlapping regions are merged once and tagged with every category they may host, and unmodified library constructs are collapsed to stubs under a rescue rule that preserves evidence. The slicer needs no compilation and processes a contract in 7.6 ms on average. On 200 contracts from the DIVE benchmark it retains 56.5\% of source characters while preserving evidence for all 463 labelled category instances, a retention of 1.00 in every category. Three arms over the same 200 contracts separate what each component contributes. Against the way such a detector is used in practice, which is to submit the complete contract with a one-line instruction per category, the decision rules raise the F1-score from 0.342 to 0.695 and cut missed vulnerabilities from 314 to 92 (McNemar exact test over 1,600 paired cells, $p \approx 7.1 \times 10^{-24}$). Adding the slicer holds that quality, 0.693 against 0.695 with no detectable difference ($p = 0.57$), while reducing token consumption by 62.0\%, cost by 63.2\% and wall-clock time by 68.8\%, to 4,689 tokens, \$0.032 and 8.4 seconds per contract. Against ordinary practice the method therefore doubles the F1-score while spending 55.2\% fewer tokens, and the result is unchanged on the 130 contracts that played no part in its development. One static artefact makes the same detector both better and cheaper.
\end{abstract}

\begin{keywords}
blockchain security, smart contract vulnerability detection, static slicing, token efficiency
\end{keywords}
...
We take the opposite route and exploit the fact that vulnerability classes have lexical footprints. A reentrancy fault requires an external call, a bad-randomness fault requires a block-derived value, and an arithmetic fault requires an operator. Code containing none of these cannot host the corresponding defect, and deciding that requires no model. This paper introduces AnchorSlice, a purely static slicing stage that keeps, for each DASP category, only the regions whose tokens make the category possible, and hands the detector a compact annotated slice instead of the full file. The design borrows the intuition of program slicing~\cite{weiser1981} but deliberately stops short of dependence analysis, so that the slicer runs in milliseconds and never needs to compile. A single pass covers all eight categories at once, and what the pass removes is bounded by a measurable guarantee: every labelled vulnerability in the evaluation keeps at least one fragment able to host it. We evaluate AnchorSlice on the DIVE multi-label benchmark~\cite{dive2026}, whose 22,330 contracts are annotated for the eight DASP categories we target.

The primary contributions of this paper are summarised as follows:
\begin{itemize}
    \item We present \emph{AnchorSlice}, a detector-agnostic, purely lexical method for token-efficient smart contract vulnerability detection, composed of two static components built from one anchor set for DASP categories 1--8: a slicer, with a visibility gate, a tag-preserving merge that emits each source region once while labelling it with every category it may host, and a library-collapse step with a rescue rule that restores lost evidence; and a set of anchor-derived decision rules that fix what each category means for the detector.
    ...
    \item We report a paired evaluation over three arms that credits each component separately: the decision rules raise the F1-score from 0.342 to 0.695 on complete sources, and the slicer then cuts token consumption by 62.0\%, cost by 63.2\% and latency by 68.8\% with no detectable change in detection quality. We test the significance of each step and analyse which anchors carried information and which misled the detector.
\end{itemize}

The remainder of this paper is structured as follows. Section~\ref{sec:related} reviews previous work on smart contract vulnerability detection and input reduction. Section~\ref{sec:anchorslice} introduces AnchorSlice and its design. Section~\ref{sec:eval} presents the experimental evaluation. Section~\ref{sec:limit} discusses limitations and threats to validity. Finally, Section~\ref{sec:conclusion} concludes the paper and outlines future work.
...
Table~\ref{tab:related_work} contrasts AnchorSlice with the closest prior work along the dimensions that matter for token-efficient detection. Rule-based analysers do not reduce their input because they do not read tokens. Learning-based detectors reduce input through slicing or graph construction, but their reduction is tied to a trained representation rather than to a vulnerability taxonomy. Prompt-compression methods reduce input for token-bound models but use a model to decide what to keep and offer no guarantee for security-relevant code. AnchorSlice is, to our knowledge, the first reduction stage that is simultaneously model-free, guided by the full vulnerability taxonomy, and validated for evidence retention on a labelled smart contract benchmark.

%============================================================
\section{AnchorSlice}\label{sec:anchorslice}
%============================================================
AnchorSlice is built from a single artefact, the anchor set of each DASP category, and turns it into two components that a source-level detector uses together. The first is a \emph{static slicer}, which keeps, for every category, the code regions able to host it, so that the detector reads far fewer tokens without losing the evidence it needs. The second is a set of \emph{anchor-derived decision rules}, generated once from the same analysis and shipped alongside the slice, which state for each category the shapes that count as an instance of it in the corpus at hand. The division of labour between them is deliberate: the slicer governs how much the detector reads and therefore what the query costs, the decision rules govern how it judges what it reads and therefore what the query is worth, and neither consumes a model call. Fig.~\ref{fig:pipeline} places both components in the detection pipeline, and Algorithm~\ref{alg:anchorslice} gives the slicing procedure, which runs in three stages: comment removal, library collapse, and anchor-guided window extraction. This section first states the principles that constrain the design (Section~\ref{subsec:design}), then describes the lexical anchors and the single semantic gate retained (Section~\ref{subsec:anchors}), the extraction and tag-preserving merge of windows (Section~\ref{subsec:merge}), library collapse and its evidence rescue rule (Section~\ref{subsec:rescue}), and finally how the slice is rendered and handed to the detector (Section~\ref{subsec:render}).

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\linewidth, keepaspectratio]{fig1_pipeline.png}
    \caption{AnchorSlice and the practice it replaces. Conventionally the whole contract is handed to the detector with a one-line list of the categories. AnchorSlice derives two static components from one anchor set per DASP category: a slicer, which emits a tagged slice of 56.5\% of the source characters while retaining all 463 labelled category instances, and a set of decision rules, which state what each category means in this corpus. The detector, Claude Opus~5, and the output schema are identical in all three arms; figures beneath each lane are per contract.}
    \label{fig:pipeline}
\end{figure*}
...
\begin{tabular}{@{}p{3.2cm} p{11.0cm} c c@{}}
\hline
...
Instruction & per arm & One-line category list (A) or anchor-derived decision rules (B, C) \\
...
The evaluation uses three arms over the same contracts, so that each component of AnchorSlice can be credited separately. Arm~A is how a source-level detector is used in practice: the complete contract with a short instruction naming the eight categories, one line each, and asking for a fault where it is reachable and exploitable. Arm~B keeps the complete contract but replaces that instruction with the anchor-derived decision rules, and therefore isolates the second component. Arm~C is AnchorSlice as proposed: the slice, preceded by a short preamble explaining the block format and stating that the category tags are lexical hints rather than findings, together with the same decision rules, and therefore adds the first component on top of Arm~B. The detector, reasoning effort, output schema and contracts are identical throughout, with one call per contract. Because the three arms classify the same 1,600 cells, every comparison is paired, and we report McNemar's exact test on discordant cells alongside the aggregate measures.
...
AnchorSlice reduced the sample from 2,311,110 to 1,305,860 characters, or 56.5\% of the original. Evidence retention was 1.00 for every category: all 463 label-positive category instances kept at least one block tagged with that category, as summarised in Table~\ref{tab:anchors}. Reaching full retention required four guards that a smaller sample had not exercised: legacy contracts in which a function without a visibility keyword is public by default, function signatures spanning several lines, anonymous fallback functions, and minified sources in which an entire contract occupies one or two lines. A contract for which no candidate block can be isolated, or whose block headers would cost more than the removed lines save, is sent whole with comments stripped, which occurred for 43 of the 200 contracts. Decomposing the reduction by stage, comment removal alone brings the sample to 77.8\% of its original size and library collapse and anchoring take it to 56.5\%, so removal accounts for most of the compression while the anchoring stage returns part of its saving in block headers. The slicer processes a contract in 7.6~ms on average and 17.5~ms at most, which is negligible beside the tens of seconds a detector call takes.

\subsection{Token Consumption, Runtime and Cost}\label{subsec:cost}
Table~\ref{tab:resources} and Fig.~\ref{fig:resources} report resource use. The decision rules are not free: stating them costs roughly two thousand tokens per call, so Arm~B reads 12,351 tokens per contract where Arm~A reads 10,465, and takes 26.8 seconds against 20.8. The slicer more than repays that. Arm~C consumes 4,689 tokens per contract, 62.0\% fewer than Arm~B and 55.2\% fewer than Arm~A, at \$0.032 against \$0.088 and \$0.089, and 8.4 seconds against 26.8 and 20.8. The contract text the detector must read, measured as the tokens written to the prompt cache, falls by 68.4\% between Arm~B and Arm~C. Output length falls with it, by 54.6\%: given a compact, tagged input the detector writes shorter justifications, so the saving compounds on the side that is billed most heavily. Over the whole sample this is \$6.45 against \$17.52 for the same decision rules on complete sources.
...
\hline
\textbf{Measure} & \textbf{Arm A} & \textbf{Arm B} & \textbf{Arm C} & \textbf{C vs B} \\
\hline\hline
Input tokens & 1,837,975 & 2,222,346 & 825,285 & $-$62.9\% \\
Output tokens & 254,939 & 247,767 & 112,469 & $-$54.6\% \\
Contract text cached & 1,100,024 & 1,072,714 & 339,432 & $-$68.4\% \\
\textbf{Total tokens} & \textbf{2,092,914} & \textbf{2,470,113} & \textbf{937,754} & \textbf{\textminus62.0\%} \\
Tokens per contract & 10,465 & 12,351 & 4,689 & $-$62.0\% \\
Wall clock (s) & 4,151 & 5,363 & 1,673 & $-$68.8\% \\
Seconds per contract & 20.8 & 26.8 & 8.4 & $-$68.8\% \\
Cost (USD) & 17.75 & 17.52 & 6.45 & $-$63.2\% \\
Cost per contract & \$0.089 & \$0.088 & \$0.032 & $-$63.2\% \\
\hline
\end{tabular}
\end{table}
...
\begin{figure*}[t]
    ...
    \caption{Resource use and detection quality per contract across the three arms. The decision rules raise both token use and latency; the slicer removes far more than they add, at the same detection quality.}
    \label{fig:resources}
\end{figure*}

\subsection{Detection Effectiveness}\label{subsec:effectiveness}
Table~\ref{tab:effectiveness} separates the two components. Replacing the one-line instruction with the anchor-derived decision rules, while still submitting the complete contract, moves every aggregate measure at once: recall rises from 0.322 to 0.801, precision from 0.365 to 0.614, F1-score from 0.342 to 0.695, Cohen's kappa from 0.097 to 0.547 and agreement from 0.642 to 0.797. Missed vulnerabilities fall from 314 to 92 while false positives fall from 259 to 233, so both error types improve together and the movement is not a shift of the decision threshold. Over the 1,600 paired cells, 185 are correct only in Arm~A and 433 only in Arm~B, a two-sided McNemar exact $p$ of about $7.1 \times 10^{-24}$. The detection quality of the method is therefore attributable to its second component.

Adding the slicer leaves that quality intact. Arm~C reaches an F1-score of 0.693 against 0.695, an agreement of 0.801 against 0.797 and a kappa of 0.549 against 0.547, and the paired test finds no difference: 53 cells are correct only with the complete source and 60 only with the slice, $p = 0.57$. It trades twelve recovered vulnerabilities for nineteen suppressed false positives, and raises the number of contracts labelled correctly in all eight categories from 25 to 28. Read together with Table~\ref{tab:resources}, this is the central result of the paper: removing 43.5\% of the source characters, and with them 62.0\% of the tokens, two thirds of the latency and 63.2\% of the cost, costs nothing in detection quality that the experiment can measure.

Against ordinary practice, the method as a whole raises F1-score from 0.342 to 0.693 and cuts missed vulnerabilities from 314 to 104 while spending 55.2\% fewer tokens ($p \approx 3.7 \times 10^{-26}$). The 130 contracts of the final sampling round, which were never inspected while the anchors and the decision rules were developed, give the same picture on their own: an F1-score of 0.689 and an agreement of 0.797, against 0.340 and 0.634 for ordinary practice, so the result is not an artefact of development on the evaluation set.
...
\hline
\textbf{Metric} & \textbf{Arm A} & \textbf{Arm B} & \textbf{Arm C} \\
\hline\hline
Label agreement & 0.642 & 0.797 & 0.801 \\
Exact match (8/8) & 5/200 & 25/200 & 28/200 \\
Precision & 0.365 & 0.614 & 0.627 \\
Recall & 0.322 & 0.801 & 0.775 \\
F1-score & 0.342 & 0.695 & 0.693 \\
Cohen's kappa & 0.097 & 0.547 & 0.549 \\
True positives & 149 & 371 & 359 \\
False positives & 259 & 233 & 214 \\
False negatives & 314 & 92 & 104 \\
\hline
\end{tabular*}
\vspace{2pt}
\parbox{\linewidth}{\scriptsize Arm~A: complete source, one-line instruction. Arm~B: complete source, anchor-derived decision rules. Arm~C: AnchorSlice, slice and decision rules. A~vs~B $p \approx 7.1 \times 10^{-24}$; B~vs~C $p = 0.57$; A~vs~C $p \approx 3.7 \times 10^{-26}$.}
\end{table}

\subsection{Per-Category Behaviour}\label{subsec:percat}
Table~\ref{tab:percat} and Fig.~\ref{fig:percat} report every category for the three arms. The decision rules account for the movement, and it is concentrated where the one-line instruction is most conservative. Reentrancy rises from an F1-score of 0.19 to 0.82 and access control from 0.41 to 0.82: asked only for faults that are reachable and exploitable, the detector marked 10 of 98 reentrancy contracts and 42 of 144 access-control contracts, whereas a rule that states what the corpus counts as an instance, supported by the \texttt{MUTFN} anchor over state-mutating entry points, recovers most of them. Unchecked return values rise from 0.46 to 0.80 and DoS from 0.17 to 0.42.

Adding the slice leaves most categories within a point or two of Arm~B and redistributes a little error: DoS and bad randomness gain, arithmetic and unchecked return values lose, and reentrancy, access control and time manipulation are unchanged in substance. Two categories resist both components. Time manipulation falls from 0.67 in Arm~A to about 0.59, because the rule recovers misses, from 15 to 4, while raising false positives from 11 to roughly 50. Front running falls to zero in both Arm~B and Arm~C: the rule suppresses the ERC-20 approve pattern that dominates the corpus, cutting 132 false positives to 16 and 14 respectively, but recovers none of its 15 positives. That category is produced by a single analyser in the reference and carries no lexical signal a detector can act on, as Section~\ref{sec:limit} discusses.
...
\hline
\textbf{Category} & \textbf{DIVE +} & \textbf{F1 (A)} & \textbf{F1 (B)} & \textbf{F1 (C)} & \textbf{FN (A)} & \textbf{FN (B)} & \textbf{FN (C)} \\
\hline\hline
Reentrancy & 98 & 0.19 & 0.82 & 0.82 & 88 & 1 & 2 \\
Access Control & 144 & 0.41 & 0.82 & 0.83 & 102 & 12 & 9 \\
Arithmetic & 84 & 0.44 & 0.57 & 0.52 & 53 & 28 & 38 \\
Unchecked Return Values & 38 & 0.46 & 0.80 & 0.74 & 20 & 10 & 13 \\
DoS & 26 & 0.17 & 0.42 & 0.47 & 19 & 16 & 15 \\
Bad Randomness & 16 & 0.58 & 0.58 & 0.62 & 9 & 9 & 8 \\
Front Running & 15 & 0.09 & 0.00 & 0.00 & 8 & 15 & 15 \\
Time manipulation & 42 & 0.67 & 0.60 & 0.58 & 15 & 1 & 4 \\
\hline
\textbf{All categories} & \textbf{463} & \textbf{0.342} & \textbf{0.695} & \textbf{0.693} & \textbf{314} & \textbf{92} & \textbf{104} \\
\hline
\end{tabular*}
\vspace{2pt}
\parbox{\linewidth}{\scriptsize DIVE + counts reference positives; FN counts missed vulnerabilities. Arm~A: complete source with the one-line instruction. Arm~B: complete source with the anchor-derived decision rules. Arm~C: AnchorSlice, slice and decision rules together.}
\end{table*}

\begin{figure*}[t]
    \centering
    \includegraphics[width=\linewidth, keepaspectratio]{fig2_per_category.png}
    \caption{F1-score per DASP category for the three arms, ordered by the size of the change between Arm~A and Arm~C. The decision rules account for nearly all of the movement; adding the slice leaves each category within a few points of Arm~B.}
    \label{fig:percat}
\end{figure*}

\subsection{Discussion}\label{subsec:discussion}
Three findings follow from Tables~\ref{tab:resources} and~\ref{tab:effectiveness}, one for each thing the experiment varies.

The first concerns what the detector is told. A one-line definition per category leaves a capable model markedly conservative: asked to report faults that are reachable and exploitable, it recovers under a third of the labelled vulnerabilities. Stating, per category, which shapes the corpus counts as an instance nearly doubles the F1-score without a change of model, of schema, or of the code shown. Much of the headroom usually attributed to model capability is therefore headroom in the instruction.

The second concerns what the detector reads. Removing 43.5\% of the source characters left every aggregate measure unchanged, and the paired test cannot distinguish the sliced arm from the one that reads the complete source. The simplest reading is that the removed text was never used: comment blocks, licence headers and unmodified library implementations carry no information on which a security judgement depends. This is consistent with the observation that evidence buried inside long inputs is attended to less reliably than evidence near the edges~\cite{liu2024lost}: a shorter input raises the density of security-relevant material and shortens the distance between related fragments. The slicer therefore buys efficiency rather than accuracy, which is what a preprocessing stage should do.

The third concerns how the two combine. The decision rules cost about two thousand tokens per call, so on complete sources they make the query more expensive, 12,351 tokens against 10,465, and slower, 26.8 seconds against 20.8. The slicer returns that outlay several times over, ending at 4,689 tokens and 8.4 seconds, so the pair is cheaper than the practice it replaces as well as more accurate. Neither half achieves this alone: rules without a slice leave the token cost untouched, and a slice without rules leaves the detector guessing what the reference counts as a vulnerability.

The stage decomposition adds a final observation about the anchors themselves. Most of the compression comes from removal, and the anchoring stage returns part of its saving in block headers, so the value of anchoring lies less in compression than in localisation. An anchor is then useful in proportion to how selective it is: the two categories whose tags fire on roughly one block in eight, bad randomness and time manipulation, are the two the one-line instruction already handled best, whereas the categories whose tags fire on more than half of all blocks are those where the decision rule, rather than the tag, carries the improvement.

\section{Limitations and Threats to Validity}\label{sec:limit}
%============================================================
The results above come from one detector, one benchmark and a single run per arm, and several properties of the design and of the evaluation bound how far they generalise. This section states these limits explicitly so that the reported numbers are interpreted within them. It first sets out what the three-arm design does and does not separate and why the monetary saving travels less well than the token count, then the two categories the per-category analysis shows the method fails to handle, and finally the threats arising from how evidence retention was validated, from the sample and the number of runs, and from the single detector used.

\smallskip
\noindent\emph{What the three arms do and do not separate.} The design isolates each component once: Arm~B against Arm~A measures the decision rules on complete sources, and Arm~C against Arm~B measures the slicer under fixed rules. It does not separate the two effects the slice itself combines, namely removing code and naming candidate categories in the block headers; a tag-free slice would be needed for that, and the opposite movements of DoS and unchecked return values suggest that these two effects can act against each other, so the aggregate comparison may understate the benefit of removal alone. Nor does it establish that the components behave independently on other corpora, since both derive from the same anchor set and are calibrated against the same reference.
...
\smallskip
\noindent\emph{Two categories the method does not solve.} Front running is the clearest failure: once the decision rules are in force the detector marks none of its 15 positives, trading the baseline's 132 false positives for 16 in Arm~B and 14 in Arm~C at the cost of every true detection. In the reference, that category is produced by a single analyser, and no lexical pattern we measured predicts it: the ERC-20 approve allowance race, the textbook shape, appears in 164 of the 200 contracts and predicts the label with a precision of 0.04. Time manipulation moves the other way, cutting missed vulnerabilities from 15 to 4 at the cost of raising false positives from 11 to 50. Relatedly, library collapse identifies constructs by name, so a contract that reuses a standard name for a modified implementation would have that code collapsed as well.
...
\smallskip
\noindent\emph{The nature of the reference.} DIVE's labels are produced by six analysers that are standard practice in this field -- MAIAN, Mythril, Semgrep, Slither, Solhint and VeriSmart -- combined by power-based voting and then re-checked against category-specific code evidence, a validation that corrected about 14.3\% of DoS and 24.9\% of time-manipulation labels~\cite{dive2026}. It is therefore a carefully constructed reference rather than an unfiltered tool dump, and reproducing it closely is a meaningful result; it is nonetheless a tool consensus rather than an audited ground truth, so agreement with it measures reproduction of what these analysers collectively find, and our decision rules follow that consensus where it departs from Solidity semantics. A benchmark with human-audited labels would test the same method against a different notion of correctness.

\smallskip
\noindent\emph{One detector.} All three arms use a single proprietary model accessed through an interface that does not expose sampling parameters or allow a dated model snapshot to be pinned. Although both components are detector-agnostic by construction, their effect on other source-level detectors, including open-weight models and multi-stage agentic pipelines~\cite{ma2025iaudit}, has not been measured.

%============================================================
\section{Conclusion and Future Work}\label{sec:conclusion}
%============================================================
This paper addressed the token cost of smart contract vulnerability detection and showed that a large share of that cost can be removed before the detector runs. We presented AnchorSlice, a static method that reduces a Solidity contract to the fragments capable of hosting each DASP category and tells the detector what each category means there, and evaluated both of its components on the DIVE benchmark. This section summarises the findings and the directions they open.

AnchorSlice is lexical throughout, needs no compilation, and processes a contract in 7.6~ms on average; both of its components are derived from one anchor set and neither costs a model call. On 200 contracts it retained evidence for all 463 labelled category instances, a retention of 1.00 in every category, while emitting 56.5\% of the original characters. Three arms over the same contracts credit each component separately. The decision rules raise the F1-score from 0.342 to 0.695 on complete sources, cutting missed vulnerabilities from 314 to 92 ($p \approx 7.1 \times 10^{-24}$), and the slicer then removes 62.0\% of the tokens, 63.2\% of the cost and 68.8\% of the latency with no detectable change in detection quality ($p = 0.57$). Against ordinary practice the method doubles the F1-score while spending 55.2\% fewer tokens, and the 130 contracts never used in development give the same result. Because both components are static, they compose at no inference cost: a practitioner who today pastes a contract into a detector can adopt AnchorSlice without changing the detector, the schema, or anything else in the pipeline.

The limitations above mark out four directions, the first two of which would sharpen the present result while the last two would establish how far it travels. A tag-free arm would separate the effect of removing code from the effect of naming candidate categories, and would also remove the header overhead that currently offsets the anchoring stage. A selectivity threshold that suppresses any category tagging more than a set fraction of a contract's blocks would address the DoS failure directly while preserving the access-control gain. The anchors should be re-validated on a held-out DIVE split, with the protocol repeated across several runs, before the approach is applied to the full corpus. Finally, the slice should be evaluated in front of other detectors, including open-weight models and multi-stage agentic pipelines, to establish that the token savings transfer across detection architectures.
