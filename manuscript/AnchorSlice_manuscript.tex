...
\begin{itemize}
    ...
    \item We validate the slicer against the DIVE benchmark and show that it retains evidence for all 119 labelled category instances in our sample while emitting 60.2\% of the original characters at an average of 7.6~ms per contract. We also report which semantic restrictions are compatible with the benchmark's annotation policy and which are not.
    \item We report a paired evaluation in which only the detector input changes. Slicing cut token consumption by 56.6\% and detection runtime by 19.7\% while raising recall, F1-score and chance-corrected agreement, and we test the significance of these changes and analyse which anchors carried useful information and which misled the detector.
\end{itemize}
...
\begin{figure*}[htbp]
    ...
    \caption{Vulnerability detection with and without AnchorSlice. Arm~A passes the complete source to the detector; Arm~B inserts the three AnchorSlice stages S1--S3, annotated with the share of corpus characters retained after each. The detector (Claude Opus~5 in this study), its prompt, and its output schema are identical in both arms.}
    \label{fig:pipeline}
\end{figure*}
...
\hline
Reentrancy & \texttt{EXT} & view/pure & 22/22 \\
Access Control & \texttt{modifier}\sep\texttt{only\textbackslash w+}\sep\texttt{(\_)owner =}\sep\texttt{tx.origin}\sep\texttt{selfdestruct}\sep\texttt{suicide}\sep\texttt{delegatecall}\sep\texttt{require(msg.sender}\sep\texttt{renounce}\sep\texttt{transferOwnership}\sep\texttt{authoriz}\sep\texttt{admin}\sep\texttt{MUTFN} & view/pure & 39/39 \\
Arithmetic & \texttt{[+ - * / \%]=}\sep\texttt{++}\sep\texttt{-{}-}\sep\texttt{x [+ * / \%] y}\sep\texttt{x - y}\sep\texttt{**}\sep\texttt{<{}<}\sep\texttt{>{}>}\sep\texttt{unchecked} & -- & 23/23 \\
Unchecked Return Values & \texttt{EXT} & view/pure & 5/5 \\
DoS & \texttt{for(}\sep\texttt{while(}\sep\texttt{.push(}\sep\texttt{.length}\sep\texttt{require(}\sep\texttt{revert}\sep\texttt{assert(}\sep\texttt{EXT} & view/pure & 7/7 \\
Bad Randomness & \texttt{blockhash}\sep\texttt{block.\{difficulty, coinbase, gaslimit, number, timestamp, prevrandao\}}\sep\texttt{now}\sep\texttt{keccak256}\sep\texttt{sha3(}\sep\texttt{sha256}\sep\texttt{random}\sep\texttt{nonce}\sep\texttt{seed} & -- & 5/5 \\
Front Running & \texttt{approve(}\sep\texttt{allowance}\sep\texttt{msg.value}\sep\texttt{tx.gasprice}\sep\texttt{price}\sep\texttt{bid}\sep\texttt{swap*(}\sep\texttt{slippage}\sep\texttt{amountOutMin}\sep\texttt{deadline}\sep\texttt{reserve}\sep\texttt{MUTFN} & view/pure & 5/5 \\
Time manipulation & \texttt{now}\sep\texttt{block.timestamp}\sep\texttt{block.number}\sep\texttt{N \{days, hours, minutes, seconds, weeks\}}\sep\texttt{deadline}\sep\texttt{startTime}\sep\texttt{endTime}\sep\texttt{cooldown}\sep\texttt{lastClaim}\sep\texttt{timeout} & -- & 13/13 \\
\hline
\textbf{All categories} & -- & -- & \textbf{119/119} \\
\hline
...
\hline
Sample size & 50 & Seeded stratified sample, seed 20260827 \\
...
We drew 50 contracts from DIVE~\cite{dive2026} using a seeded random sample that was repaired so that every category has at least five positive and five negative instances. This keeps base rates close to the corpus prevalence while making all eight categories measurable in both directions; a sample composed only of heavily labelled contracts would leave precision undefined, because a detector answering all-positive would score perfectly. Contracts smaller than 1,200 bytes were excluded as empty stubs, and contracts larger than 30,000 bytes were excluded to bound the cost of each call. The sample totals 552,043 characters, with a median contract of 11,484 characters and a maximum of 27,687, so no contract required truncation. Compiler versions split 27 legacy contracts against 23 at Solidity~0.8 or above. The 400 label cells (50 contracts $\times$ 8 categories) contain 119 positives, a positive rate of 29.8\%.

\subsection{Evaluation Protocol}\label{subsec:protocol}
Arm~A submits the complete source of each contract. Arm~B submits the AnchorSlice output preceded by a short preamble explaining the block format and stating that the category tags are lexical hints, not findings. Everything else is held constant: the same detector, reasoning effort, system prompt, output schema and contracts, with one call each. The system prompt defines each DASP category with a minimal code example and calibrates the detector's decision threshold to the benchmark, instructing it to mark a category when the vulnerable pattern is present and not demonstrably mitigated. Because both arms classify the same 400 cells, the comparison is paired, and we report McNemar's exact test on discordant cells alongside the aggregate measures.
...
AnchorSlice reduced the sample from 552,043 to 332,134 characters, or 60.2\% of the original, with a mean per-contract reduction of 34.0\%. Evidence retention was 1.00 for every category: all 119 label-positive category instances kept at least one block tagged with that category, as summarised in Table~\ref{tab:anchors}. Decomposing the pipeline in Fig.~\ref{fig:pipeline} shows that comment removal brings the corpus to 76.3\% and library collapse to 56.0\%, with the anchoring stage returning 60.2\% after its block headers are counted. The slicer processed the 50 contracts in 378.5~ms, a mean of 7.6~ms and a maximum of 17.5~ms per contract, averaged over five passes, which is negligible beside the tens of seconds a detector call takes.

\subsection{Token Consumption, Runtime and Cost}\label{subsec:cost}
Table~\ref{tab:resources} reports resource use. Slicing cut total token consumption from 913,958 to 396,438, a reduction of 56.6\%, and mean consumption per contract from 18,279 to 7,928 tokens. Wall-clock time fell from 1,019.6 to 818.5 seconds, or 19.7\%, which matters for throughput when detection is scaled beyond a sample; in both arms the interface-reported inference time accounted for essentially all of the wall-clock time. Output tokens fell by 15.6\%, a smaller proportion than the input reduction, because the length of the detector's reasoning is governed by the eight decisions it must make rather than by how much source it reads. Cost decreased by only 1.4\%, for reasons discussed in Section~\ref{sec:limit}.

\begin{table}[t]
\centering
\caption{Resource Consumption Across 50 Contracts}
\label{tab:resources}
\footnotesize
\renewcommand{\arraystretch}{1.2}
...
Cache-write tokens & 125,385 & 176,438 & $+$40.7\% \\
Cache-read tokens & 724,571 & 166,026 & $-$77.1\% \\
\textbf{Total tokens} & \textbf{913,958} & \textbf{396,438} & \textbf{\textminus56.6\%} \\
Tokens per contract & 18,279 & 7,928 & $-$56.6\% \\
Wall clock (s) & 1,019.6 & 818.5 & $-$19.7\% \\
Seconds per contract & 20.4 & 16.4 & $-$19.7\% \\
...
Detection quality improved on every aggregate measure, as Table~\ref{tab:effectiveness} shows. Recall rose from 47.9\% to 63.0\%, an absolute gain of 15.1 percentage points, and precision rose from 38.5\% to 41.4\%, so the additional detections were not obtained by indiscriminate over-reporting. F1-score improved from 42.7\% to 50.0\%, and Cohen's kappa from 0.145 to 0.220, a relative gain of 51.7\%. Label agreement rose from 61.8\% to 62.5\%, and Hamming loss fell correspondingly. Two contracts were classified correctly in all eight categories under slicing, whereas the baseline achieved none.

The confusion matrix locates the change. False negatives fell from 62 to 44 while true positives rose from 57 to 75, against an increase in false positives from 91 to 106. At contract level, 14 contracts improved, 23 were unchanged and 13 regressed. The gain is therefore concentrated in recovering vulnerabilities the baseline missed rather than in eliminating spurious reports. These aggregate improvements are not statistically significant: over the 400 paired cells, 20 cells were correct only under the baseline and 23 only under slicing, giving a two-sided McNemar exact $p$ of 0.761.

\begin{table}[t]
\centering
\caption{Detection Effectiveness Over 400 Label Cells}
...
Table~\ref{tab:percat} reports every category, and Fig.~\ref{fig:percat} shows the three that improved. Access control gained 14.0 percentage points, by far the largest movement, and is also the category in which the baseline was weakest, marking only 6 of 39 positives. The \texttt{MUTFN} anchor tags precisely the state-mutating entry points where a missing modifier would live, and under slicing the detector identified 11 more, cutting false negatives from 33 to 24 at a cost of two false positives. Front running gained 6.0 points and eliminated both of its false negatives, and reentrancy gained 4.0 points while reducing false negatives from four to one. Unchecked return values and time manipulation held their agreement, and the former halved its false negatives, so six of the eight categories either improved or held. Arithmetic and bad randomness each lost 2.0 points, and DoS lost 14.0 points, which we attribute to an over-broad anchor discussed in Section~\ref{sec:limit}.
...
\hline
\textbf{Category} & \textbf{DIVE +} & \textbf{Detector + (A)} & \textbf{Detector + (B)} & \textbf{Agreement (A)} & \textbf{Agreement (B)} & \textbf{Change (pp)} & \textbf{FN (A)} & \textbf{FN (B)} \\
\hline\hline
...
\hline
\textbf{All categories} & \textbf{119} & \textbf{148} & \textbf{181} & \textbf{0.618} & \textbf{0.625} & \textbf{+0.7} & \textbf{62} & \textbf{44} \\
\hline
\multicolumn{9}{@{}p{16.5cm}@{}}{\scriptsize DIVE + counts reference positives; Detector + counts contracts the detector marked positive; FN counts false negatives. Change is the shift in label agreement in percentage points.} \\
\end{tabular}
\end{table*}
...
\begin{figure}[t]
    ...
    \caption{Label agreement with and without slicing for the three categories in which agreement improved. Table~\ref{tab:percat} reports all eight categories.}
    \label{fig:percat}
\end{figure}

\subsection{Discussion}\label{subsec:discussion}
The central observation is that discarding roughly 40\% of the source characters did not degrade the detector's judgement and, on aggregate measures, improved it. The simplest reading is that the removed text was not being used. Comment blocks, licence headers and unmodified library implementations carry no information on which a security judgement depends, and the detector appears to treat them accordingly. This is consistent with the observation that evidence buried inside long inputs is attended to less reliably than evidence near the edges~\cite{liu2024lost}: shortening the input raises the density of security-relevant material and shortens the distance between related fragments. The stage decomposition adds a second insight. Comment removal and library collapse account for essentially all of the size reduction, while the anchoring stage removes 22.3\% of the remaining source and adds 16.9\% back as block headers. The value of anchoring therefore lies less in compression than in localisation: the tags tell the detector where each category could plausibly live, which suggests that an anchor is useful in proportion to how selective it is.
...
AnchorSlice retained 60.2\% of the characters of the 50-contract sample, with 14.9 blocks per contract, and every one of the 119 label-positive category instances kept at least one block tagged with its category. Comment removal and library collapse delivered most of the reduction, and the rescue rule was necessary to reach full retention after collapse. The slicer needs no compilation and processes a contract in 7.6~ms on average.
...
Slicing reduced total token consumption by 56.6\%, from 18,279 to 7,928 tokens per contract, and wall-clock detection time by 19.7\%. The reduction fell mainly on cache-read input tokens, which dropped by 77.1\%, while output tokens fell by 15.6\%, and monetary cost fell by only 1.4\% because the removed tokens belong to the cheapest token class.
...
Under slicing, recall rose by 15.1 percentage points, precision by 2.9, F1-score by 7.3 and Cohen's kappa from 0.145 to 0.220, with false negatives falling from 62 to 44. Access control, front running and reentrancy improved; unchecked return values and time manipulation held; arithmetic and bad randomness declined slightly; and DoS declined by 14.0 points because its anchor fires almost everywhere. The paired McNemar test does not reach significance ($p = 0.761$).
...
\noindent\emph{Statistical strength.} The aggregate detection improvements are not statistically significant. McNemar's exact test over the 400 paired cells gives 20 cells correct only under the baseline against 23 correct only under slicing, for a two-sided $p$ of 0.761. The gains in recall and F1-score should therefore be read as encouraging rather than established; the claim the experiment supports firmly is that slicing does not harm detection while more than halving token use.

\noindent\emph{Token savings versus cost savings.} The monetary saving is much smaller than the token saving. Cost fell by 1.4\% against a 56.6\% token reduction because the reduction falls almost entirely on cached input, the cheapest token class, while output tokens changed comparatively little. Detectors whose pricing weights these classes differently will see a different figure, so token count and latency are the more portable benefits.

\noindent\emph{An over-broad anchor.} One anchor is clearly mis-specified. The DoS anchor includes \texttt{require}, \texttt{revert} and \texttt{assert}, which appear in nearly every non-trivial function, so almost every block carries a DoS tag. The detector read that ubiquity as evidence and raised its positive count from 13 to 20 against a reference of 7, without recovering any false negative. A tag that fires almost everywhere carries no information yet still shifts the detector's prior. Relatedly, library collapse identifies constructs by name, so a contract that reuses a standard name for a modified implementation would have that code collapsed as well.
...
\noindent\emph{In-sample development and single runs.} The anchors were developed by iterating against the same 50 contracts used for evaluation, so the retention figure is an in-sample fit and will be optimistic on held-out data. The sample is also small, and each arm was run once, so part of the discordance between arms reflects the detector's own non-determinism.

\noindent\emph{One detector.} Both arms use a single proprietary model accessed through an interface that does not expose sampling parameters or allow a dated model snapshot to be pinned. Although the slice is detector-agnostic by construction, its effect on other source-level detectors, including open-weight models and multi-stage pipelines such as LineGuard~\cite{lineguard}, has not been measured.
...
AnchorSlice is lexical throughout, needs no compilation, and processes a contract in 7.6~ms on average. On a stratified sample of 50 contracts it retained evidence for all 119 labelled category instances while emitting 60.2\% of the original characters. In a paired evaluation it reduced token consumption by 56.6\% and detection runtime by 19.7\%, while raising recall from 47.9\% to 63.0\%, F1-score from 42.7\% to 50.0\% and Cohen's kappa from 0.145 to 0.220. These detection gains are not statistically significant, and the results come from a single run of one detector on an in-sample set of anchors, but they indicate that taxonomy-aware input reduction is compatible with, and can benefit, source-level vulnerability detection.
